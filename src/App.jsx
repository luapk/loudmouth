...
/*
  ...
  Harvests real Reddit posts and comments server-side, clusters them
  with Claude into stable tensions and live expressions, gates every
  expression against the product truths, and attaches the receipts:
  actual quotes linked to their threads.
*/
...
const SCAN_STAGES = [
  ...
  "Harvesting Reddit, YouTube and TikTok in parallel",
  "Pulling comment layers from the loudest threads",
  ...
  const runScan = useCallback(async () => {
    ...
    try {
      ...
      setData((d) => ({
        ...d,
        [market]: {
          scannedAt: `Live scan · ${c.reddit || 0} Reddit · ${c.youtube || 0} YouTube · ${c.tiktok || 0} TikTok trends · ${new Date().toLocaleString("en-GB", { day: "numeric", month: "short", hour: "2-digit", minute: "2-digit" })}`,
    ...
    >
      ...
      <div style={{ maxWidth: 1180, margin: "0 auto" }}>
        <div style={{ display: "flex", flexWrap: "wrap", alignItems: "flex-end", justifyContent: "space-between", gap: 18, marginBottom: 26 }}>
          <div>
            <div className="mono" style={{ fontSize: 10, letterSpacing: "0.28em", color: "#6EA8FF", marginBottom: 8 }}>
              ORAL-B iO · CULTURAL SENSING ENGINE · v3 · REDDIT + YOUTUBE + TIKTOK INGESTION
            </div>
        ...
        <div className="mono" style={{ fontSize: 10, marginTop: 30, letterSpacing: "0.12em", color: "#5A6885", lineHeight: 1.8 }}>
          LOUDMOUTH v3 · EVIDENCE HARVESTED LIVE FROM REDDIT, YOUTUBE COMMENTS AND TIKTOK CREATIVE CENTER · CULTURE SPECIMENS FROM LIVE WEB SEARCH, VERIFY BEFORE ANY DECK · INSIGHTS SHIP WITH EXPIRY DATES BY DESIGN
        </div>
      </div>
    </div>
  );
